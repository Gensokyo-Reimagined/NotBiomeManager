        ...
        private static void applyConfigTo(ConfigurationNode root, SpecialEffectsBuilder specialEffectsBuilder) {
                ...
                compute(legacyNode.node("Grass_Modifier"), x -> specialEffectsBuilder
                                .grassColorModifier(BiomeSpecialEffects.GrassColorModifier
                                                .valueOf(x.getString().toUpperCase())));
                ...
                compute(legacyNode.node("Dry_Foliage_Color"),
                                x -> specialEffectsBuilder
                                                .dryFoliageColorOverride(fromRgbString(x.getString()).asRGB()));
                ...
                compute(attributesNode.node("minecraft:visual/cloud_height"),
                                x -> specialEffectsBuilder.cloudHeight(x.getDouble()));

                // Cloud Fog End Distance
                compute(legacyNode.node("Cloud_Fog_End_Distance"),
                                x -> specialEffectsBuilder.cloudFogEndDistance(x.getDouble()));
                compute(attributesNode.node("minecraft:visual/cloud_fog_end_distance"),
                                x -> specialEffectsBuilder.cloudFogEndDistance(x.getDouble()));

                // Fog Start Distance
                compute(legacyNode.node("Fog_Start_Distance"),
                                x -> specialEffectsBuilder.fogStartDistance(x.getDouble()));
                compute(attributesNode.node("minecraft:visual/fog_start_distance"),
                                x -> specialEffectsBuilder.fogStartDistance(x.getDouble()));

                // Fog End Distance
                compute(legacyNode.node("Fog_End_Distance"), x -> specialEffectsBuilder.fogEndDistance(x.getDouble()));
                compute(attributesNode.node("minecraft:visual/fog_end_distance"),
                                x -> specialEffectsBuilder.fogEndDistance(x.getDouble()));

                // Sky Fog End Distance
                compute(legacyNode.node("Sky_Fog_End_Distance"),
                                x -> specialEffectsBuilder.skyFogEndDistance(x.getDouble()));
                compute(attributesNode.node("minecraft:visual/sky_fog_end_distance"),
                                x -> specialEffectsBuilder.skyFogEndDistance(x.getDouble()));

                // Sky Light Color
                compute(legacyNode.node("Sky_Light_Color"),
                                x -> specialEffectsBuilder.skyLightColor(fromRgbString(x.getString()).asRGB()));
                compute(attributesNode.node("minecraft:visual/sky_light_color"),
                                x -> specialEffectsBuilder.skyLightColor(fromRgbString(x.getString()).asRGB()));

                // Sky Light Factor
                compute(legacyNode.node("Sky_Light_Factor"), x -> specialEffectsBuilder.skyLightFactor(x.getDouble()));
                compute(attributesNode.node("minecraft:visual/sky_light_factor"),
                                x -> specialEffectsBuilder.skyLightFactor(x.getDouble()));
                ...
                compute(attributesNode.node("minecraft:visual/star_brightness"),
                                x -> specialEffectsBuilder.starBrightness(x.getDouble()));
                ...
                compute(attributesNode.node("minecraft:visual/particle"), x -> {
                        ...
                        compute(actualNode.node("options", "type"), typeNode -> {
                                ...
                                ParticleOptions nmsParticle = null;
                                try {
                                        nmsParticle = ParticleArgument
                                                        .readParticle(new StringReader(typeNode.getString()), access);
                                } catch (CommandSyntaxException e) {
                                        logger.warning("Exception encountered for particle " + e.toString());
                                }
                                specialEffectsBuilder.ambientParticleParticleOptions = Optional.ofNullable(nmsParticle);
                        ...
                        compute(actualNode.node("probability"),
                                        prob -> specialEffectsBuilder.ambientParticleProbability = Optional
                                                        .of(prob.getFloat()));
                });
                ...
                compute(legacyMoodNode.node("Sound"),
                                x -> specialEffectsBuilder.ambientMoodSoundEvent = Optional
                ...
                compute(attributesNode.node("minecraft:audio/mood_sound"), x -> {
                        ...
                        compute(actualNode.node("block_search_extent"),
                                        s -> specialEffectsBuilder.ambientMoodBlockSearchExtent = Optional
                                                        .of(s.getInt()));
                        compute(actualNode.node("offset"),
                                        s -> specialEffectsBuilder.ambientMoodSoundPositionOffset = Optional
                                                        .of(s.getDouble()));
                });

                // Ambient Additions
                var legacyAdditionsNode = legacyNode.node("Random_Sound");
                compute(legacyAdditionsNode.node("Sound"),
                                x -> specialEffectsBuilder.ambientAdditionsSoundEvent = Optional
                                                .ofNullable(getSoundFromKey(x.getString())));
                ...
                compute(attributesNode.node("minecraft:audio/additions_sound"), x -> {
                        ConfigurationNode actualNode = x.node("default").virtual() ? x : x.node("default");
                        compute(actualNode.node("sound"),
                                        s -> specialEffectsBuilder.ambientAdditionsSoundEvent = Optional
                                                        .ofNullable(getSoundFromKey(s.getString())));
                        compute(actualNode.node("tick_chance"),
                                        s -> specialEffectsBuilder.ambientAdditionsTickChance = Optional
                                                        .of(s.getDouble()));
                });
                ...
                Consumer<ConfigurationNode> musicParser = (x) -> {
                        ...
                        if (!actualNode.isList()) {
                                compute(actualNode.node("Sound"),
                                                y -> specialEffectsBuilder.singleBackgroundMusicSoundEvent = Optional
                                ...
                                compute(actualNode.node("Min_Delay"),
                                                y -> specialEffectsBuilder.singleBackgroundMusicMinDelay = Optional
                                                                .of(y.getInt()));
                                compute(actualNode.node("min_delay"),
                                                y -> specialEffectsBuilder.singleBackgroundMusicMinDelay = Optional
                                                                .of(y.getInt()));
                                ...
                                compute(actualNode.node("Max_Delay"),
                                                y -> specialEffectsBuilder.singleBackgroundMusicMaxDelay = Optional
                                                                .of(y.getInt()));
                                compute(actualNode.node("max_delay"),
                                                y -> specialEffectsBuilder.singleBackgroundMusicMaxDelay = Optional
                                                                .of(y.getInt()));
